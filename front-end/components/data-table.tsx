...
import SearchOrdersForm from "./search-orders-form"
...
const columns: ColumnDef<z.infer<typeof schema>>[] = [
  {
    ...
    header: "Nome",
    cell: ({ row }) => {
      return <p className="w-64">{row.original.nome}</p> 
    },
    enableHiding: false,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const isDone = row.original.statusPagamento === "Concluído"
  ...
  {
    accessorKey: "data",
    header: () => <div className="w-full text-left">Data</div>,
    cell: ({ row }) => (
      <p>{row.original.data}</p>
    ),
  },
]
